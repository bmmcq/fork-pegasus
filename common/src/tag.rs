...
mod tests {
    ...
    fn tag_test() {
        ...
        assert_eq!(tag1.to_parent().unwrap(), null());
        assert_eq!(null().to_parent(), None);
    }
    ...
    fn tag_serialize() {
        ...
        assert_eq!(Tag::read_from(&mut reader).expect("read tag failure"), Tag::Null);
