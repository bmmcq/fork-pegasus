...
use crate::operators::repeat::switch::unary::RepeatSwitchUnaryOperatorBuilder;
...
impl<D: Data> Stream<D> {
    ...
    async fn _switch(
        ...
        let worker_index = self.builder.cluster_peer_index;
        let into_repeat =
            MultiScopeStreamBuilder::<D>::new(worker_index, scope_ctx.level(), (new_op_index, 1).into());
    ...
    fn new_repeat_scope(&self) -> ScopeContext {
        let id = self.builder.new_scope_id();
        self.scope_ctx.new_repeat(id)
    }
...
impl DataflowBuilder {
        ...
        It::IntoIter: Send + 'static,
    {
        ...
        Stream {
            ...
            tag: Tag::Null,
            scope_ctx: ScopeContext::new(0, 0, ContextKind::Flat),
            src: output_builder,
            channel: ChannelKind::Pipeline,
            builder: self.clone(),
        }
