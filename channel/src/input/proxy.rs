...
impl <T> InputProxy<T> where T: Data {
    ...
    pub fn downcast(input: &Box<dyn AnyOutput>) -> Option<RefMut<InputHandle<T, BasePull<MiniScopeBatch<T>>>>> {
        input.as_any_ref().downcast_ref::<Self>().map(|i| i.0.borrow_mut())
    }
}

impl <T> Input for InputProxy<T> where T: Data {
...
impl <T> MultiScopeInputProxy<T> where T: Data {
    ...
    pub fn downcast(input: &Box<dyn AnyOutput>) -> Option<RefMut<MultiScopeInputHandle<T, BasePull<MiniScopeBatch<T>>>>> {
        input.as_any_ref().downcast_ref::<Self>().map(|i| i.0.borrow_mut())
    }
}

impl <T> Input for MultiScopeInputProxy<T> where T : Data {
    fn info(&self) -> InputInfo {
        self.0.borrow().info()
    }
