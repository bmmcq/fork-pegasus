...
impl<T> MultiScopeBatchDecoder<T> {
    pub fn new(scoped_pool: Arc<SharedScopedBufferPool<T>>) -> Self {
        Self { binded: AHashMap::new(), scoped_pool, notify: Arc::new(Notify::new()) }
    }
    ...
    T: Decode + Data,
{
    ...
    async fn decode(&mut self, mut bytes: Bytes) -> Result<Self::Item, Error> {
        ...
        if len > 0 {
            ...
            if let Some(eos) = eos {
                batch.set_end(eos);
                let (tag, b) = self.binded.remove_entry(batch.tag()).expect("");
                self.scoped_pool
                    .release_slot_async(&tag, b)
                    .await;
            }
            Ok(batch)
        } else {
            // len == 0;
            if let Some(eos) = eos {
                let mut batch = MiniScopeBatch::<T>::new(tag, src, RoBatch::default());
                batch.set_end(eos);
                Ok(batch)
            } else {
                unreachable!("")
