    ...
    P: Push<MicroBatch<T>>,
{
    fn push(&mut self, tag: &Tag, msg: T) -> IOResult<Pushed<T>> {
        ...
        match self.buffer.add(msg) {
            Ok(Some(batch)) => {
                let batch = MicroBatch::new(tag.clone(), self.worker_index, batch);
    ...
    fn push_last(&mut self, msg: T, mut end: Eos) -> IOResult<()> {
        ...
        let mut batch = MicroBatch::new(end.tag.clone(), self.worker_index, batch);
    ...
    fn notify_end(&mut self, mut end: Eos) -> IOResult<()> {
        ...
        let mut batch = if let Some(batch) = self.buffer.exhaust() {
            let mut batch = MicroBatch::new(self.tag.clone(), self.worker_index, batch);
            self.total_send += batch.len();
            batch
        } else {
            MicroBatch::new(self.tag.clone(), self.worker_index, RoBatch::default())
        };
        end.total_send = self.total_send as u64;
        batch.set_end(end);
    ...
    fn flush(&mut self) -> IOResult<()> {
        if let Some(buf) = self.buffer.flush() {
            let batch = MicroBatch::new(self.tag.clone(), self.worker_index, buf);
            self.total_send += batch.len();
            self.inner.push(batch)?;
            self.inner.flush()
        } else {
